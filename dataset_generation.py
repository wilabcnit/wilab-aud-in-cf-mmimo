...
def main():
    ...
    files = int(training_samples/batch_size) # Number of files for training data
    L = 32.4 + 20.0*np.log10(f) # Path loss constant
    
    
    # Pilots/Preamble generation
    S = (np.random.normal(size=(tau,K)) + 1j*np.random.normal(size=(tau,K)))*np.sqrt(var_s/2.0)
    ...
    pilots_feat = folder + "/pilots"+ str(M) + ".pkl"
    ...
    # Saving preamble sequence
    S_flat = np.concatenate( (np.real(S), np.imag(S)), axis = 0) #(2*tau, K)
    pilots_df = pd.DataFrame(S_flat, columns = pilots)
    pilots_df.to_pickle(pilots_feat)
    
    # Generating and saving training samples
    for i in range(files):
        train_feat = folder + "/train_feat_MU_" + str(M) + "_"+ str(i)+ ".pkl"
        train_out = folder + "/train_out_MU_" + str(M) + "_"+ str(i)+ ".pkl"
        features, labels = generate_samples(batch_size, L, D, M, K, N, S, D_p, tau, std_shadowing, var_h,var_n,eta)
                                          
        train_df = pd.DataFrame(features, columns = feature)
        train_df.to_pickle(train_feat)
        
        train_df = pd.DataFrame(labels, columns = outputs)
    ...
    # Generating and saving validation samples
    features, labels = generate_samples(valid_samples, L, D, M, K, N, S, D_p, tau, std_shadowing, var_h,var_n,eta)
    valid_df = pd.DataFrame(features, columns = feature)
    valid_df.to_pickle(valid_feat)
    
    valid_df = pd.DataFrame(labels, columns = outputs)
    valid_df.to_pickle(valid_out)
